...
export class CosmosRestClient extends BaseRestClient<RequestRegistry> {
  ...
  async getEstimateProviderRewards(address: string, amount: string) {
    return this.request({url: "/lavanet/lava/subscription/estimated_provider_rewards/{address}/{amount}", adapter} as Request<{rewards: any}>, { address, amount });
  }
 ...
 async getLavaIPRPCRewardsPools() {
    return this.request({url: "/lavanet/lava/rewards/iprpc_spec_reward/", adapter} as Request<{iprpc_rewards: any[]}>, {});
 } 
 async getLavaPools() {
    return this.request({url: "/lavanet/lava/rewards/pools", adapter} as Request<any>, {});
 }
 // todo: use const for url
